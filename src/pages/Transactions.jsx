...
import { Search, Download, Plus, X, ClipboardList, Pencil, Trash2, Calendar as CalendarIcon, Filter } from 'lucide-react';
...
const Transactions = ({ isDarkMode }) => {
  ...
  // DATE PICKER STATE (Array: [start, end])
  ...
  const [formData, setFormData] = useState({
    date: new Date(), // Now using Date Object for Picker
    itemName: "",
    type: "IN",
    quantity: "",
    altQty: "",
  ...
  const openEditModal = (txn) => {
    ...
    setFormData({
      ...txn,
      date: new Date(txn.date) // Convert string back to Date object for picker
    });
    setIsModalOpen(true);
  };
  ...
  const filteredTransactions = transactions.filter(txn => {
    ...
    if (startDate && endDate) {
      ...
      // Reset times to ensure accurate day comparison
    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
        <div>
          ...
          <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Track stock movements</p>
        </div>
      </div>
      ...
      <div className={`p-4 rounded-t-xl flex flex-col md:flex-row flex-wrap gap-4 justify-between items-center border-b ${isDarkMode ? 'bg-darkcard border-gray-700' : 'bg-white border-gray-200'}`}>
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto items-stretch md:items-center">
            ...
            <div className="relative w-full md:w-64">
              ...
              <input type="text" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={`w-full pl-10 pr-4 py-2 rounded-lg border focus:ring-2 focus:ring-blue-500 outline-none ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'}`} />
            </div>

            {/* ENHANCED DATE RANGE PICKER */}
            <div className="relative w-full md:w-64 z-20">
              <div className="absolute left-3 top-3 pointer-events-none z-10 text-gray-400">
              ...
              <DatePicker
                ...
                className={`w-full pl-10 pr-4 py-2 rounded-lg border cursor-pointer focus:ring-2 focus:ring-blue-500 outline-none ${isDarkMode ? 'bg-gray-800 border-gray-700 text-white placeholder-gray-400' : 'bg-gray-50 border-gray-200 text-gray-800'}`}
        ...
        <div className="flex gap-3 w-full md:w-auto justify-end">
          ...
          <button onClick={openAddModal} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow-md transition-all text-sm md:text-base">
            <Plus size={18} /> <span className="hidden md:inline">Add Transaction</span><span className="md:hidden">Add</span>
          </button>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className={`border-b ${isDarkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-100 bg-gray-50'}`}>
              {["Date", "Type", "Item", "Qty", "Alt Qty", "Remarks"].map(h => (
                ...
                {userRole === 'admin' && (
                  <td className="p-4">
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => handleDelete(txn.id)} className="p-2 hover:bg-red-100 text-red-600 rounded-lg transition-colors"><Trash2 size={16} /></button>
                    </div>
                  </td>
                )}
              </tr>
            ...
            <div className="overflow-y-auto p-6">
              <form onSubmit={handleSave} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* ENHANCED DATE PICKER FOR FORM */}
                  <div className="flex flex-col">
                    <label className="block text-sm font-medium mb-1">Date</label>
                    <DatePicker 
                      selected={formData.date} 
                      onChange={(date) => setFormData({...formData, date})} 
                      className={`w-full p-3 rounded-lg border outline-none focus:ring-2 focus:ring-blue-500 ${isDarkMode ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-800'}`}
                      dateFormat="yyyy-MM-dd"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1">Type</label>
                    <select value={formData.type} onChange={(e) => setFormData({...formData, type: e.target.value})} className="w-full p-3 rounded-lg border dark:bg-gray-800 dark:border-gray-700 outline-none focus:ring-2 focus:ring-blue-500">
                      <option value="IN">Stock IN (Purchase)</option>
                      <option value="OUT">Stock OUT (Use)</option>
                    </select>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input required list="items" value={formData.itemName} onChange={handleItemSelect} className="w-full p-3 rounded-lg border dark:bg-gray-800 dark:border-gray-700 outline-none focus:ring-2 focus:ring-blue-500" placeholder="Type to search..." />
                ...
                <div>
                  ...
                  <textarea value={formData.remarks} onChange={(e) => setFormData({...formData, remarks: e.target.value})} className="w-full p-3 rounded-lg border dark:bg-gray-800 dark:border-gray-700 outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Purchase order #123" rows="2"></textarea>
                </div>
                ...
                <div className="pt-4 flex gap-3">
                  ...
                  <button type="submit" className="flex-1 py-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-lg transition-all">{editingId ? "Update" : "Save"}</button>
                </div>
              </form>
            </div>
          </div>
      ...
      {/* --- DATEPICKER DARK MODE & MOBILE OVERRIDES --- */}
      <style>{`
        ...
        .dark .react-datepicker__day--selected, .dark .react-datepicker__day--in-selecting-range, .dark .react-datepicker__day--in-range, .dark .react-datepicker__month-text--selected, .dark .react-datepicker__month-text--in-selecting-range, .dark .react-datepicker__month-text--in-range, .dark .react-datepicker__quarter-text--selected, .dark .react-datepicker__quarter-text--in-selecting-range, .dark .react-datepicker__quarter-text--in-range, .dark .react-datepicker__year-text--selected, .dark .react-datepicker__year-text--in-selecting-range, .dark .react-datepicker__year-text--in-range { background-color: #2563eb; color: white; }
        .dark .react-datepicker__day--keyboard-selected, .dark .react-datepicker__month-text--keyboard-selected, .dark .react-datepicker__quarter-text--keyboard-selected, .dark .react-datepicker__year-text--keyboard-selected { background-color: #1d4ed8; color: white; }
        /* Close Button fix */
